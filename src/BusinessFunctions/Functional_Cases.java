...
public class Functional_Cases {
	...
	public void CNetLogin(WebDriver driver) throws AWTException, InterruptedException, IOException {
		try {
			...
			fL.Invokeapplication(driver, prop.getProperty("BaseUrl"), prop.getProperty("Browser"),
					prop.getProperty("BaseUrl"), "Passing URL in to browser", "URL Should be passed in to browser",
					"URL Launched successfully", "N");

			SwitchToMainFrame(driver, prop.getProperty("MainframeID"));

			fL.clickByID(driver, prop.getProperty("UserNameID"), prop.getProperty("UserNameID"), "Click on Username",
					"Username should be clicked", "UserName clicked successfully", "N");

			fL.enterValuebyID(driver, prop.getProperty("UserNameID"), "maneeshn", "maneeshn",
					"Passing value in to username", "Username should be entered",
					"Username should be passed successfully", "N");

			fL.clickByID(driver, prop.getProperty("PasswordID"), prop.getProperty("PasswordID"), "Click on Password",
					"Password should be clicked", "Password clicked successfully", "N");

			fL.enterValuebyID(driver, prop.getProperty("PasswordID"), "Maneeshnama@6468", "Maneeshnama@6468",
					"Passing value in to Password", "Password should be entered",
					"Password should be passed successfully", "N");

			fL.clickByID(driver, prop.getProperty("LoginButtonID"), prop.getProperty("LoginButtonID"),
					"Click on Login button", "Login button should be clicked", "Login button clicked succesfully", "N");
	...
	public void AIMSLaunchApplication(WebDriver driver) throws AWTException, InterruptedException, IOException {
		try {
			...
			fL.enterValuebyID(driver, prop.getProperty("FilterID"), "AIMS", prop.getProperty("FilterID"),
					"Passed AIMS in FilterNavigator", "Find AIMS in search Navigator", "Search Successful", "N");

			fL.clickByID(driver, prop.getProperty("LaunchApplicationID"), prop.getProperty("LaunchApplicationID"),
					"clicked on launch Application", "Launch AIMS Application", "Clicked Successful", "N");
	...
	public void UpdateVirtualMachine_Update(WebDriver driver, String ProjectInformation, String Project,
			String PrimaryDatacenter, String PrimaryCloudPlatform, String VcenterHost, String ESXIHOST,
		...
		try {
			...
			fL.selectDropdownbyID(driver, prop.getProperty("ServerSize"), Serversize, Serversize,
					"Selected Service Size", "Select one Size", "Select Successful", "N");

			if (Storage.equalsIgnoreCase("Yes")) {

				fL.selectDropdownbyID(driver, prop.getProperty("AdditionalStorage"), Storage,
						prop.getProperty("AdditionalStorage"), "Selected Additional Storage",
						"You need addition storage??", "Yes Selected", "N");

				fL.selectDropdownbyID(driver, prop.getProperty("StorageProviderID"), StorageProvier,
						prop.getProperty("StorageProviderID"), "Selected NetApp Storage provider",
						"Select Storage Provider", "Select Successfull", "N");

				fL.enterValuebyID(driver, prop.getProperty("StorageNameID"), StorageName,
						prop.getProperty("StorageNameID"), "Provided Storage Name", "Provide Storge name", "Successful",
						"N");

				fL.enterValuebyID(driver, prop.getProperty("AggregateNameID"), AggregateName,
						prop.getProperty("AggregateNameID"), "Provided AggregateName", "Provide Aggregate name",
						"Successful", "N");

				fL.enterValuebyID(driver, prop.getProperty("VserverID"), VServer, prop.getProperty("VserverID"),
						"Provided VServerId", "Provide VServerId", "Successful", "N");

				fL.enterValuebyID(driver, prop.getProperty("SizeID"), Size, prop.getProperty("SizeID"), "Entered 30",
						"prvoide Size", "Successful", "N");

				fL.selectDropdownbyID(driver, prop.getProperty("SizeUnitID"), SizeUnits, prop.getProperty("SizeUnitID"),
						"Selected SizeUnit", "Select SizeUnit", "Successful", "N");

				fL.SwitchFrames(driver, prop.getProperty("MainframeID"), prop.getProperty("MainframeID"),
						"Swicthing to Main Frame", "Main Frame should be activited", "Mainframe actitivited", "N");

				// fL.selectDropdownbyID(driver, prop.getProperty("ProtocolID"),
				// "Custom",prop.getProperty("ProtocolID"), "Selected Additional
				// Storage", "You need addition storage??", "Yes Selected",
				// "N");

				FirewalPortRules(driver, Protocol, OpenPort, Port, AllowForm, IP, FromPort, ToPort, FromIP, ToIP);

				MonitoringInformation(driver, AddMonitering, InstallAgentMonitering, InstandEnablSNMP,
						ChangeDefaultCommString, CommunityString);

				// fL.clickByID(driver,
				// prop.getProperty("OrderNow"),prop.getProperty("OrderNow")
				// ,"Clicked OrderNow", "Place Your order", "Successful", "N");

			} else {

				fL.selectDropdownbyID(driver, prop.getProperty("AdditionalStorage"), "No", "No",
						"Value need to be Selected from the dropdown", "No value shouldbe selected",
						"No value isselected", "N");
	...
	public void ImpersonateUser(WebDriver driver) throws AWTException, InterruptedException, IOException {
		try {
			...
			fL.clickByID(driver, prop.getProperty("infoDropdownid"), prop.getProperty("infoDropdownid"),
					"Clicked User Menu", "Select An User", "Select Successful", "N");

			fL.ClickByLinkText(driver, prop.getProperty("ImpersonateUserLink"), prop.getProperty("ImpersonateUserLink"),
					"Select ImpersonateUser", "Choose An User", "User Select Successful", "N");

			fL.clickByxpath(driver, prop.getProperty("SearchUserButtonXpath"),
					prop.getProperty("SearchUserButtonXpath"), "Clicked Search Bar", "Slect Search for User",
					"Successful", "N");

			fL.enterValuebyID(driver, prop.getProperty("SearchUserID"), "Maneesh Nama", "Maneesh Nama",
					"Passed UserName", "Provide UserName", "Successfully Provided", "N");

			fL.clickByClass(driver, prop.getProperty("SearchLabelUserClasss"),
					prop.getProperty("SearchLabelUserClasss"), "Clicked SearchLabel", "Click search for user",
					"Successful", "N");
	...
	public void RequestApproval(WebDriver driver, String RequestNumber, String replaceReqNum)
			throws AWTException, InterruptedException, IOException {
		try {
			...
			fL.enterValuebyID(driver, prop.getProperty("FilterID"), "Requests", prop.getProperty("FilterID"),
					"passed the requests in FilterNavigator", "Search for requests", "Succesfully passed", "N");

			fL.clickByID(driver, prop.getProperty("SearchRequestID"), prop.getProperty("SearchRequestID"),
					"Clicked on Service catalog Requests", "Click on ServiceCatalog Requests", "Successful", "N");
			...
			fL.enterValuebyXpath(driver, prop.getProperty("RequestSearchxpath"), RequestNumber,
					prop.getProperty("RequestSearchxpath"), "Passed The RequestNumber", "Search for Requests",
					"Successful", "N");

			fL.PressEnterKeyUsingXpath(driver, prop.getProperty("RequestSearchxpath"));

			fL.ClickByLinkText(driver, RequestNumber, RequestNumber, "Clicked RequestNumber", "Click on request Number",
					"Successful", "N");
			...
			fL.ClickByLinkText(driver, replaceReqNum, replaceReqNum,
					"Clicking on Request Number Which contains RITM after replacing REQ to RITM", "Replace ReqNum",
					"Successful", "N");
			...
			fL.selectDropdownbyID(driver, prop.getProperty("ApprovedID"), "Approved", prop.getProperty("ApprovedID"),
					"Changed request to approve", "aprrove status", "aproved successfully", "N");

			fL.clickByID(driver, prop.getProperty("UpdateButtonID"), prop.getProperty("UpdateButtonID"), "Updated form",
					"click update button", "Successfull", "N");
			...
			fL.ClickByLinkText(driver, replaceReqNum, replaceReqNum, "Replaced he request from REQ to RITM",
					"Replace RITM", "Successful", "N");

			fL.SwitchFrames(driver, prop.getProperty("MainframeID"), prop.getProperty("MainframeID"),
					"Swicthing to Main Frame", "Main Frame should be activited", "Mainframe actitivited", "N");

			fL.clickByxpath(driver, prop.getProperty("ApprovalTabxpath"), prop.getProperty("ApprovalTabxpath"),
					"Clicked on approval tab on RITM", "Manager Approval", "Successful", "N");
	...
	public void ShowWorkFlow(WebDriver driver) throws AWTException, InterruptedException, IOException {
		try {
			...
			fL.clickByID(driver, prop.getProperty("showworkflowID"), prop.getProperty("showworkflowID"),
					"Clicked on Workflow in RITM Form", "Show the workflow", "Workflow clicked Successful", "N");

			fL.SwitchtoAnotherWindows(driver, prop.getProperty("UpdateServerLink"),
					"Selected updateserver tab on workflow", "click on updateserver tab", "successful", "N");
	...
	public String ReplaceRequestNumber(WebDriver driver, String RequestNumber)
	...
	public void ApprovalRequestTabLinkText(WebDriver driver, String locator) throws IOException {
		...
		FluentWait<WebDriver> waitforelement = new FluentWait<WebDriver>(driver).withTimeout(60, TimeUnit.SECONDS)
				.pollingEvery(10, TimeUnit.SECONDS).ignoring(NoSuchElementException.class);
		waitforelement.until(ExpectedConditions.presenceOfElementLocated(By.linkText(locator)));
	...
	public void Search_cmdb_ci_server_list(WebDriver driver) throws IOException, InterruptedException {
		...
		try {
			...
			fL.enterValuebyXpath(driver, prop.getProperty("FilterXpath"), "cmdb_ci_server.list", "cmdb_ci_server.list",
					"passed cmdb server list in filternavigator", "pass cmdb server table in search navigator",
					"Successfully passed and entered", "N");
	...
	public void FirewalPortRules(WebDriver driver, String Protocol, String OpenPort, String Port, String FromPort,
			String ToPort, String AllowForm, String IP, String FromIP, String ToIP)
			throws AWTException, InterruptedException, IOException {
		try {
	...
	public void MonitoringInformation(WebDriver driver, String AddMonitering, String InstallAgentMonitering,
			String InstandEnablSNMP, String ChangeDefaultCommString, String CommunityString)
			throws AWTException, InterruptedException, IOException {
		try {
